...
async function setStatus (token, endpoint, status) {
  ...
  let { data } = await axios.post(config.urls.HVACSetModes, {
    ...
    flags: 255,
    endpointID: endpoint,
    mode: mode ? mode.code : MODES.find(m => m.name === 'NO_CHANGE').code,
    fanMode: fanMode ? fanMode.code : FAN_MODES.find(m => m.name === 'NO_CHANGE').code,
...
function parseStatus (data) {
  ...
  try {
    ...
    throw new Error(`Error parsing packet for device: ${error.message}`)
  }
