...
async function setStatus (token, endpoint, status) {
  let { data } = await axios.post(config.urls.HVACSetModes, {
    ...
    flags: 255,
    endpointID: endpoint,
    mode: MODES.find(m => m.name === status.mode).code,
    fanMode: FAN_MODES.find(m => m.name === status.fanMode).code,
...
async function turnOn (token, endpoint) {
  let success = await setStatus(token, endpoint, {
    mode: MODES.find(m => m.name === 'AUTO').code
  })
  return success
}
...
async function turnOff (token, endpoint) {
  let success = await setStatus(token, endpoint, {
    mode: MODES.find(m => m.name === 'OFF').code
  })
  return success
}
