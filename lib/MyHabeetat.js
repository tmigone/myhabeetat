...
const MYHABEETAT_LOGIN = 'https://myhabeetat.solidmation.com/accounts/login.aspx'
const MYHABEETAT_CLOUD = 'https://myhabeetatcloud-services.solidmation.com/1.0/HomeCloudService.svc'
...
async function login (email, password) {
  let { data } = await axios.post(MYHABEETAT_LOGIN, { AccountEmail: email, Password: password })
...
async function getHomes (token) {
  let { data } = await axios.post(`${MYHABEETAT_CLOUD}/EnumHomes`, { token: { Token: token } })
...
async function getDataPacket (token, home) {
  let { data } = await axios.post(`${MYHABEETAT_CLOUD}/GetDataPacket`, {
    homeID: home,
    token: { Token: token },
    timeOut: 10000,
...
async function getDeviceStatus (token, home, device) {
  ...
  switch (_device.DeviceModelID) {
    case DEVICE_MODELS.BGH_SMART_CONTROL:
      data = BGHSmartControl.parse(endpoint.Values)
      break
    default:
